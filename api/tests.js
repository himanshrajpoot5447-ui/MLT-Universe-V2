...
export default handler(async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // Ensure marking columns exist (safe on any schema version)
    try {
      await query(`ALTER TABLE tests ADD COLUMN IF NOT EXISTS positive_marks NUMERIC(5,2) NOT NULL DEFAULT 1`);
      await query(`ALTER TABLE tests ADD COLUMN IF NOT EXISTS negative_marks NUMERIC(5,2) NOT NULL DEFAULT 0`);
      await query(`ALTER TABLE tests ADD COLUMN IF NOT EXISTS assigned_question_ids JSONB DEFAULT '[]'`);
      await query(`ALTER TABLE tests ADD COLUMN IF NOT EXISTS attempts_count INTEGER NOT NULL DEFAULT 0`);
      await query(`ALTER TABLE tests ADD COLUMN IF NOT EXISTS avg_score INTEGER NOT NULL DEFAULT 0`);
