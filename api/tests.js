...
export default handler(async (req, res) => {
  ...
  if (id) {
    ...
    if (req.method === 'DELETE') {
      ...
      // Fetch XP earned from attempts before deleting test
      let testAttempts = [];
      try { testAttempts = await query('SELECT user_id, xp_earned FROM attempts WHERE test_id=$1', [id]); } catch {}

      // Delete credit transactions linked to this test (debit transactions when test was started)
      try { await query("DELETE FROM credit_transactions WHERE description ILIKE $1 OR note ILIKE $1", ['%'+id+'%']); } catch(e) { console.warn('[tests/delete] credit_tx cleanup:', e.message); }

      // Delete test — CASCADE removes attempts
      await query('DELETE FROM tests WHERE id=$1', [id]);

      // Reverse XP for all users who attempted this test
      if (testAttempts.length > 0) {
        try {
          for (const att of testAttempts) {
            const xpToRemove = Math.max(0, att.xp_earned || 0);
            if (xpToRemove > 0) {
              const u = await queryOne('SELECT total_xp FROM users WHERE id=$1', [att.user_id]);
              if (u) await query('UPDATE users SET total_xp=$1 WHERE id=$2', [Math.max(0,(u.total_xp||0)-xpToRemove), att.user_id]);
            }
          }
          const students = await query("SELECT id FROM users WHERE role='student' ORDER BY total_xp DESC");
          for (let j = 0; j < students.length; j++) await query('UPDATE users SET rank=$1 WHERE id=$2', [j+1, students[j].id]);
        } catch(xpErr) { console.warn('[tests/delete] XP reversal:', xpErr.message); }
      }
      return ok(res, { message: 'Test, all attempts and linked credit transactions deleted' });
    }
