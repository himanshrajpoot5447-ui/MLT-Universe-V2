...
export default handler(async (req, res) => {
  ...
  if (req.method === 'POST' && urlPath.endsWith('/start')) {
    ...
    if (cu.role === 'student' && test.credits > 0) {
      ...
      try {
        ...
        console.error('[attempts/start] failed to log deduction transaction:', e.message);
      }
    }
  ...
  if (req.method === 'POST') {
    ...
    // Log a transaction for this test: deduction if it cost credits, otherwise a zero-amount XP record
    // so the credit history / activity ledger is never empty after taking a test
    if (!creditAlreadyDeducted && cu.role === 'student' && test.credits > 0) {
      ...
      try {
        ...
        console.error('[attempts] failed to log deduction transaction:', e.message);
      }
      ...
      try {
        ...
        console.error('[attempts] failed to log activity transaction:', e.message);
      }
    }
    ...
    try {
      await supaRest('PATCH', `users?id=eq.${user.id}`, { total_xp: newXP });
    } catch (e) {
      console.error('[attempts] failed to update total_xp:', e.message);
    }
