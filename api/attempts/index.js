...
export default handler(async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    if (currentUser.role === 'student' && test.credits > 0) {
      await query('UPDATE users SET credits = GREATEST(0, COALESCE(credits,0) - $1) WHERE id=$2', [test.credits, user.id]);
      await query(
    ...
    // Add XP — use explicit cast to prevent type errors
    await query('UPDATE users SET total_xp = COALESCE(total_xp,0) + $1 WHERE id=$2', [xp, user.id]);
