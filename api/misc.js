...
export default handler(async (req, res) => {
  ...
  if (subpath.endsWith('/health')) {
    // Check required env vars (v2.5 — no DB URL needed, uses REST API)
    const checks = {
    ...
    try {
      ...
      return res.status(503).json({
        status: 'db_error',
        error: e.message,
        fix: 'Run the pg_query RPC in Supabase SQL Editor — see sql/schema.sql',
      });
    }
  }
  ...
  if (subpath.endsWith('/sections')) {
    ...
    if (req.method === 'POST') {
      ...
      const exists = await queryOne(
        'SELECT id FROM sections WHERE name ILIKE $1',
        [name.trim()]
      );
  ...
  if (sectionDeleteMatch) {
    ...
    if (req.method === 'DELETE') {
      ...
      const row = await queryOne(
        'SELECT * FROM sections WHERE name ILIKE $1',
        [sectionName]
      );
