...
export default handler(async (req, res) => {
  ...
  if (id) {
    ...
    if (req.method === 'PUT') {
      ...
      if (!target) return err(res, 'User not found', 404);
      // #13: Nobody can edit the master user except themselves for password only
      if (target.role === 'master' && user.id !== id) return err(res, 'Master account cannot be modified', 403);
      ...
      if (status && isAdmin(user) && target.role !== 'master') { sets.push(`status=$${i++}`); params.push(status); }
      if (credits !== undefined && isMaster(user)) { sets.push(`credits=$${i++}`); params.push(parseInt(credits)||0); }
      // #13: Prevent changing master role or any master field
      if (role && isMaster(user) && target.role !== 'master' && role !== 'master') { sets.push(`role=$${i++}`); params.push(role); }
      if (name) {
    ...
    if (req.method === 'DELETE') {
      ...
      if (!target) return err(res, 'User not found', 404);
      // Explicitly delete all related data before user (CASCADE should handle it, but be safe)
      try { await query('DELETE FROM attempts WHERE user_id=$1', [id]); } catch {}
      try { await query('DELETE FROM credit_transactions WHERE user_id=$1', [id]); } catch {}
      // Remove any references where this user allocated credits to others
      try { await query('UPDATE credit_transactions SET by_user_id=NULL WHERE by_user_id=$1', [id]); } catch {}
